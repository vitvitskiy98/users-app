...
export const requestUsersWithError = (params: Query): Promise<string> => {
  return Promise.reject("500, unknown server error");
};
...
export const requestUsers = (
  ...
  const filtered = users.filter((v) => {
    ...
    const passedNameFilter =
      params.name === "" ||
      (params.name && v.name.toLowerCase().includes(params.name.toLowerCase()));
